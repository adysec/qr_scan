...
const GUI_TIMEOUT_DEFAULT_SECS: u64 = 5;
const GUI_TIMEOUT_MAX_SECS: u64 = 180;
    ...
    tauri::async_runtime::spawn_blocking(|| {
        let files = FileDialog::new()
            .add_filter(
                "发票文件",
                &[
                    "pdf", "png", "jpg", "jpeg", "bmp", "webp", "tif", "tiff", "doc", "docx",
                ],
            )
            .set_title("选择要识别的 PDF/图片/Word 文件（可多选）")
            .pick_files();
...
#[tauri::command]
async fn scan_invoice_qr(
    app: tauri::AppHandle,
    files: Vec<String>,
    timeout_secs: Option<u64>,
) -> Result<Vec<QrRow>, String> {
    let filtered_files: Vec<PathBuf> = files
        .into_iter()
        .map(PathBuf::from)
        .filter(|p| p.exists())
        .filter(|p| is_supported_input_path(p))
        .collect();
    let timeout = Duration::from_secs(
        timeout_secs
            .unwrap_or(GUI_TIMEOUT_DEFAULT_SECS)
            .clamp(1, GUI_TIMEOUT_MAX_SECS),
    );

    let mut rows: Vec<QrRow> = Vec::new();
    let total = filtered_files.len();

    for (idx, path) in filtered_files.iter().enumerate() {
        let _ = app.emit(
            "scan-progress",
            ScanProgress {
                processed: idx,
                total,
                current_file: format!("{} (识别中)", path.display()),
                found: 0,
            },
        );

        let path_buf = path.to_path_buf();
        let outcome = tauri::async_runtime::spawn_blocking(move || scan_file_with_timeout(path_buf, timeout))
            .await
            .map_err(|e| e.to_string())?;

        let (file_row, progress_current_file, found) = match outcome {
            Ok(FileScanOutcome::Completed(file_rows)) => {
                if file_rows.is_empty() {
                    (
                        build_failed_row(path, "识别失败"),
            ...
            Ok(FileScanOutcome::TimedOut) => (
                build_failed_row(path, &format_scan_error_marker("超时")),
                format!("{} (超时跳过)", path.display()),
                0,
            ),
            Err(err) => (
                build_failed_row(path, &format_scan_error_marker(&err.to_string())),
                format!("{} (失败: {})", path.display(), err),
                0,
            ),
        };

        rows.retain(|r| r.file != path.display().to_string());
        rows.push(file_row.clone());
...
#[tauri::command]
async fn scan_invoice_qr_one(file: String, timeout_secs: Option<u64>) -> Result<QrRow, String> {
    let path = PathBuf::from(file);
    if !path.exists() {
        return Err("文件不存在".to_string());
    }
    if !is_supported_input_path(&path) {
        return Err("仅支持 PDF/图片/Word 文件".to_string());
    }
    ...
    let outcome = tauri::async_runtime::spawn_blocking(move || scan_file_with_timeout(path_for_scan, timeout))
        .await
        .map_err(|e| e.to_string())?;
    ...
    let row = match outcome {
        ...
        Err(err) => build_failed_row(&path, &format_scan_error_marker(&err.to_string())),
    };

    Ok(row)
}

fn scan_file_with_timeout(path: PathBuf, timeout: Duration) -> Result<FileScanOutcome> {
    let (tx, rx) = mpsc::channel();

    if matches!(extension_lower(&path).as_deref(), Some("pdf")) {
        // PDF text parsing is cheap and avoids expensive image extraction when payload text exists.
        let text_rows = scan_pdf_text_rows(&path)?;
        if !text_rows.is_empty() {
            return Ok(FileScanOutcome::Completed(text_rows));
        }
    }
    ...
    match rx.recv_timeout(soft_timeout) {
        Ok(result) => {
            ...
            if !rows.is_empty() || !matches!(extension_lower(&path).as_deref(), Some("pdf")) {
                return Ok(FileScanOutcome::Completed(rows));
            }

            let render_rows = scan_pdf_render_rows(&path)?;
            if !render_rows.is_empty() {
                return Ok(FileScanOutcome::Completed(render_rows));
            }

            return Ok(FileScanOutcome::Completed(rows));
        }
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            return Err(anyhow!("扫描线程异常: 通道断开"));
        }
    ...
    if remaining.is_zero() {
        return Ok(FileScanOutcome::TimedOut);
    }
    ...
    match rx.recv_timeout(remaining) {
        Ok(result) => {
            ...
            Ok(FileScanOutcome::Completed(rows))
        }
        Err(mpsc::RecvTimeoutError::Timeout) => Ok(FileScanOutcome::TimedOut),
        Err(mpsc::RecvTimeoutError::Disconnected) => Err(anyhow!("扫描线程异常: 通道断开")),
    }
}
...
#[tauri::command]
fn export_csv(rows: Vec<QrRow>) -> Result<String, String> {
    let output = FileDialog::new()
        .add_filter("CSV", &["csv"])
        .set_file_name("qr_results.csv")
        .set_title("导出识别结果")
        .save_file()
        .ok_or_else(|| "已取消导出".to_string())?;

    write_csv(&output, &rows).map_err(|e| e.to_string())?;
